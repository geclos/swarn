...
const program = Effect.gen(function* () {
	...
	if (worktreeResult?.branch) {
		...
		// Generate PR title from first task or use default
		const prTitle = plan[0]?.title ?? "Swarn automated changes";

		// Generate PR body with summary of changes
		const prBodyLines = [
			"## Summary",
			"",
			"This PR was automatically generated by Swarn.",
			"",
			"### Tasks Completed",
			"",
			...plan.map((t: PlanTask) => `- ${t.id}: ${t.title}`),
			"",
			"### Files Changed",
			"",
